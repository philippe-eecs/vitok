...
"""NaFlex (NaViT + FlexiViT) preprocessing ops for VAEs meant to preserve aspect ratio."""
...
def _enforce_aspect_ratio_constraint(image_hw, min_ratio=0.67, max_ratio=1.33):
  """Enforces aspect ratio constraints on the image dimensions.
  
  Args:
    image_hw: Image height and width.
    min_ratio: Minimum allowed aspect ratio (h/w).
    max_ratio: Maximum allowed aspect ratio (h/w).
    
  Returns:
    Adjusted image height and width that satisfy the aspect ratio constraints.
  """
  h, w = tf.cast(image_hw[0], tf.float32), tf.cast(image_hw[1], tf.float32)
  aspect_ratio = h / w
  
  # If aspect ratio is outside the allowed range, adjust dimensions
  def adjust_for_min_ratio():
    # If h/w < min_ratio, increase h or decrease w
    new_h = tf.maximum(h, w * min_ratio)
    return tf.stack([new_h, w])
  
  def adjust_for_max_ratio():
    # If h/w > max_ratio, decrease h or increase w
    new_w = tf.maximum(w, h / max_ratio)
    return tf.stack([h, new_w])
  
  adjusted_hw = tf.cond(
      aspect_ratio < min_ratio,
      adjust_for_min_ratio,
      lambda: tf.cond(
          aspect_ratio > max_ratio,
          adjust_for_max_ratio,
          lambda: tf.stack([h, w])
      )
  )
...
def _get_image_size_for_seq(
  ...
  """
  # First enforce aspect ratio constraints
  adjusted_hw = _enforce_aspect_ratio_constraint(
      image_hw, min_ratio=min_aspect_ratio, max_ratio=max_aspect_ratio)
  image_h = tf.cast(image_hw[0], tf.float32)
  image_w = tf.cast(image_hw[1], tf.float32)
  if image_h != adjusted_hw[0] or image_w != adjusted_hw[1]:
    print(f"Adjusted image size: {adjusted_hw} from {image_hw}")
  
  def search_not_done(lb, rb):
    return (rb - lb) >= eps

  def prepare_target_hw(ratio):
    target_hw = adjusted_hw * ratio
...
@Registry.register("preprocess_ops.ar_preserving_crop_to_patch_divisible")
@utils.InKeyOutKey(indefault="image", outdefault="image")
def get_ar_preserving_crop_to_patch_divisible(patch_size: int):
  """Crops image to ensure dimensions are divisible by patch_size while preserving aspect ratio.
  
  This function performs a center crop that:
  1. Maintains the original aspect ratio as closely as possible
  2. Ensures both height and width are divisible by patch_size
  3. Maximizes the resulting image size (crops the minimum amount necessary)
  
  Args:
    patch_size: Size of the patches used for patchification.
    
  Returns:
    Pre-processing op that crops the image to dimensions divisible by patch_size.
  """
  def _ar_preserving_crop(image):
    # Get current image dimensions
    image_h, image_w = tf.shape(image)[0], tf.shape(image)[1]
    
    # Calculate target dimensions that are divisible by patch_size
    # We find the largest dimensions <= original size that are divisible by patch_size
    target_h = image_h - (image_h % patch_size)
    target_w = image_w - (image_w % patch_size)
    
    # Ensure we have at least one patch in each dimension
    target_h = tf.maximum(target_h, patch_size)
    target_w = tf.maximum(target_w, patch_size)
    
    # Calculate crop offsets (for center crop)
    offset_h = (image_h - target_h) // 2
    offset_w = (image_w - target_w) // 2
    ...
    return tf.ensure_shape(cropped_image, [None, None, 3])
  
  return _ar_preserving_crop