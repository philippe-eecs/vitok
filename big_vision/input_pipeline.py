...
DEFAULT_NUM_PARALLEL_CALLS = tf.data.AUTOTUNE


def make_for_train(
    data, preprocess_fn, batch_size,
    shuffle_buffer_size=None, cache_raw=False,
    num_parallel_calls=DEFAULT_NUM_PARALLEL_CALLS, prefetch=tf.data.AUTOTUNE,
    *,
...
def training(input_config):
  ...
  if isinstance(input_config.data.get("name"), str):
    ...
    train_ds = make_for_train(
        ...
        prefetch=input_config.get("prefetch", tf.data.AUTOTUNE),  # Default AUTOTUNE
  ...
  if (pf := input_config.get("prefetch", tf.data.AUTOTUNE)): # Default AUTOTUNE
...
def make_for_inference(
  ...
  data = data.repeat()
  data = data.prefetch(prefetch)  # Use argument directly
