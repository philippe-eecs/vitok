...
def load_params(ckpt, load_ema=False, **kw):
  ...
  if isinstance(ckpt, str):  # Most common case of passing a checkpoint path.
    ...
    if ".npz" in ckpt:  # Not a perfect heuristic, but good enough.
      ...
      if "params" in checkpoint and not load_ema:
        # Checkpoint with optax state (after (internal link)).
        params = checkpoint["params"]
      elif "ema_params" in checkpoint and load_ema:
        # Checkpoint with optax state (after (internal link)).
      ...
      if not load_ema:
        ...
        regex = f"ema_params/{key}($|/.*)" if key else "ema_params/.*"
      assert "regex" not in kw, "For a custom regex, use tsload directly."
      kw["regex"] = regex
      print(f"Loading checkpoint from {ckpt} with regex {regex}")
...
def tsload(path, *, tree=None, shardings=None, regex=None):
  ...
  arrays = array_serial.run_deserialization(shardings, specs, concurrent_gb=8)
